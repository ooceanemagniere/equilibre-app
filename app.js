...
const weekStart=()=>{const d=new Date(),n=(d.getDay()+6)%7;d.setHours(0,0,0,0);d.setDate(d.getDate()-n);return d}; const thisWeek=iso=>new Date(iso)>=weekStart();
...
function qtyFor(segment, food){
 ...
 let m=s.match(/(\d+(?:[.,]\d+)?)\s*(kg|g|ml|cl)\b/);
 if(m){let v=parseFloat(m[1].replace(',','.')),u=m[2]; if(u==='kg')v*=1000;if(u==='cl')v*=10;return v}
 m=s.match(/(\d+(?:[.,]\d+)?)\s*(?:x\s*)?(?:oeufs?|kiwis?|bananes?|pommes?|oranges?|tomates?|poivrons?|carottes?|avocats?|pots?|doses?|scoops?|tranches?|tartines?)/);
 if(m){const n=parseFloat(m[1].replace(',','.'));return n*(food.piece||100)}
 // number directly before alias: "2 oeufs"
 m=s.match(/\b(\d+(?:[.,]\d+)?)\b/); if(m && food.unit==='piece') return parseFloat(m[1].replace(',','.'))*(food.piece||100);
 // practical defaults when no quantity
 if(food.unit==='piece')return food.piece||100;
 const defaults={'beurre':10,'huile olive':10,'parmesan':20,'feta':30,'amandes':25,'noix':25,'chocolat noir':20,'isolate':30,'pain':60,'pain complet':60,'skyr':140,'yaourt grec':150,'fromage blanc':150};
 return defaults[food.name]||100;
}
function analyze(text){
 const parts=text.split(/[,;+\n]|\bet\b/gi).map(s=>s.trim()).filter(Boolean),found=[];
 for(const part of parts){
  const np=norm(part); let best=null;
  for(const f of FOOD){for(const a of f.aliases){if(np.includes(norm(a)) && (!best || a.length>best.alias.length))best={food:f,alias:a}}}
  if(best){const g=qtyFor(part,best.food),k=g/100,f=best.food;found.push({name:f.name,g:Math.round(g),kcal:f.kcal*k,p:f.p*k,c:f.c*k,fat:f.f*k,fi:f.fi*k})}
 }
 const total=found.reduce((a,x)=>({kcal:a.kcal+x.kcal,p:a.p+x.p,c:a.c+x.c,fat:a.fat+x.fat,fi:a.fi+x.fi}),{kcal:0,p:0,c:0,fat:0,fi:0});
 Object.keys(total).forEach(k=>total[k]=Math.round(total[k]*10)/10);
 return {found,total,unmatched:parts.filter(p=>!found.some(x=>norm(p).includes(norm(x.name.split(' ')[0]))))};
}
...
function foodAdvice(t){
 ...
 else if(t.p<75)a.push(['Protéines en bonne voie','Tu as déjà une bonne base. Un prochain repas avec 20–30 g de protéines complètera facilement la journée.']);
 ...
 if(t.kcal<900)a.push(['Journée encore légère','Les apports enregistrés sont encore assez légers. Ne cherche pas à “économiser” ton dîner si tu as faim.']);
 return a.slice(0,3);
}
function recipes(t){
 const arr=[];
 if(t.p<60)arr.push(['Poulet, semoule & légumes','≈ 25 min','Une source de protéines claire + légumes + féculents selon ta faim.']);
 if(t.fi<20)arr.push(['Bowl lentilles, feta, tomates & avocat','≈ 15 min','Riche en fibres, rassasiant et facile à préparer.']);
 if(t.kcal<1000)arr.push(['Pâtes courgettes, poulet & parmesan','≈ 20 min','Un vrai repas complet et réconfortant si ta journée a été légère.']);
 arr.push(['Omelette, pommes de terre & salade','≈ 15 min','Simple, rassasiant et facile à adapter.'],['Saumon, riz & brocoli','≈ 25 min','Protéines, féculents et légumes dans une assiette très simple.'],['Skyr, banane, avoine & amandes','≈ 5 min','Idéal pour une collation ou un petit-déjeuner nourrissant.']);
...
function coachState(w,m,wb,t){
 ...
 if(t.p<50)return['Ta journée a surtout besoin de simplicité.','Pour le prochain repas, pense d’abord à une vraie source de protéines et à quelque chose qui te rassasie.'];
...
function render(){
 const now=new Date(),days=Math.max(0,Math.floor((now-new Date(data.startDate))/86400000)),ww=data.workouts.filter(w=>thisWeek(w.date)),tm=data.meals.filter(m=>m.date.slice(0,10)===today()),supp=data.supplements[today()]||{},routine=['creatine','magnesium'].filter(k=>supp[k]).length,wb=data.wellbeing[today()],tot=dayTotals(tm);
 ...
 const pct=Math.min(100,Math.round((Math.min(ww.length,2)/2)*70+Math.min(tm.length,3)/3*20+routine/2*10));$('weeklyWorkoutText').textContent=`${ww.length}/2`;$('weekPercent').textContent=`${pct}%`;$('weekRing').style.background=`conic-gradient(var(--accent) ${pct*3.6}deg,var(--soft) ${pct*3.6}deg)`;$('todayMeals').textContent=tm.length;$('todayRoutine').textContent=`${routine}/2`;$('todayMood').textContent=wb?`${wb.mood}/5`:'—';
 if(wb)['energy','mood','sleep'].forEach(k=>{$(k).value=wb[k];$(k+'Val').textContent=`${wb[k]}/5`});document.querySelectorAll('.routine').forEach(b=>{const d=!!supp[b.dataset.supp];b.classList.toggle('done',d);b.querySelector('i').textContent=d?'✓':'○'});
 ...
 $('recipeIdeas').innerHTML=recipes(tot).map(r=>`<div class="recipe"><b>${r[0]}</b><small>${r[1]}</small><p>${r[2]}</p></div>`).join('');
 $('sportWeekTitle').textContent=`${ww.length}/2 séances`;$('sportStatusBadge').textContent=ww.length>=2?'Objectif atteint ✨':ww.length===1?'Encore 1':'À commencer';$('sportProgress').style.width=`${Math.min(100,ww.length/2*100)}%`;$('workoutList').innerHTML=data.workouts.length?data.workouts.slice().reverse().slice(0,8).map(w=>`<div class="mealItem"><b>${w.type}</b><p>${new Date(w.date).toLocaleDateString('fr-FR')} · ${w.duration} min · ${w.feeling}</p></div>`).join(''):'<div class="empty">Aucune séance enregistrée.</div>';
